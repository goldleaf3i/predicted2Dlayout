...
def estrai_celle_esterne_confinanti(plan_o,spazio):
	cellette_out = plan_o.cellette_esterne
	celle_confinanti = []
	for c in spazio.cells:
		for co in cellette_out:
			if fc.adiacenti(c.c,co.c):
				celle_confinanti.append(co)
	return celle_confinanti
...
def possibili_combinazioni(set):
	'''
	creo le possibili combinazioni
	'''
	combinazioni = []
	if len(set) == 0:
		return combinazioni
	for i in range (1,len(set)+1):
		for elem in combinations(set,i):
