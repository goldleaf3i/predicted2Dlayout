...
def mean_shift(h,minOffset,walls):
	...
	'''
	#----Nuova
	new_cluster_centers,max_diff = calcola_new_cluster_centers(h,cluster_centers,directions)
	#------
...
def calcola_new_cluster_centers(h,cluster_centers,directions):
	...
	for alfa in cluster_centers:
		...
		for teta in directions:
			...
			denominatore += kernel
		'''
		new=numeratore/denominatore
		if(abs(new-alfa)>max_diff):
			max_diff=abs(new-alfa)
		#dopo aver calcolato numeratore e denominatore aggiungo il nuovo cluster_center alla lista new_cluster_centers
		new_cluster_centers.append(new)
	return new_cluster_centers,max_diff

#integrata all'interno di calcola_new_cluster_centers
