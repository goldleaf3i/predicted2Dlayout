...
def contorno_esterno_versione_tre(img_rgb):
	...
	img_to_contours = thresh.copy()
	if cv2.__version__[0] == '3' :
		im_copy,contours, hierarchy = cv2.findContours(img_to_contours,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
	elif cv2.__version__[0] == '2' :
		contours, hierarchy = cv2.findContours(img_to_contours,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
