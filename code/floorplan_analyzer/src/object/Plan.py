...
class ListOfPoint(object):
	...
	def getXMin(self):
		return min(self.listOfPoint)
	def getXMax(self):
		return max(self.listOfPoint)
	def getYMin(self):
		return min(self.listOfPoint,key=itemgetter(1))
	def getYMax(self):
		return max(self.listOfPoint,key=itemgetter(1))
...
def unisciOversegmentazione(plan_o, parametri_obj):
	'''
...
def trova_spazi_parziali_da_frontiere(plan_o, lista_pixel_frontiere, immagine_cluster, labels):
	'''
	terzo metodo per riconoscere le stanze parziali, basato sul riconoscimento delle frontiere.
	Questo metodo controlla che all'interno dei pixel di una cella ci siano almeno il 30% di pixel dello stesso cluster per fare in modo che quella cella sia una cella parziale
	'''
	soglia_pixel = 5 #15 prima della modifica
	...
	for spazio in spazi:
		for celletta in spazio.cells:
			...
			try:
				...
				larghezza = 0
				print celletta.cella
				print "ERROR, minx = ",bordo[0], " maxx = ", bordo[2]
				print "ERROR, miny = ",bordo[1], " maxy = ", bordo[3]
			total_count_pixel = 0
			pixel_diversi = 0
			...
			if pixel_diversi >= soglia_pixel: #elimino un po' di rumore
...
def trova_spazi_parziali_da_frontiere_online(plan_o, lista_r,lista_g,lista_b,lista_cluster, immagine_cluster):
	'''
	terzo metodo per riconoscere le stanze parziali, basato sul riconoscimento delle frontiere.
	Questo metodo controlla che all'interno dei pixel di una cella ci siano almeno il 30% di pixel dello stesso cluster per fare in modo che quella cella sia una cella parziale
	'''
			...
			for i in xrange(0, altezza):
				for j in xrange( 0, larghezza):
					...
					if r!=g or r!=b or g!=b:#se ci sono componenti rgb diverse allora non sono in scala di grigio e quel pixel fa parte della frontiera
						...
						n = 0
						trovato = False
						while n < len(lista_r) and trovato == False:
							re = lista_r[n]
							gr = lista_g[n]
							bl = lista_b[n]
							if r == re and g == gr and b == bl:
								trovato = True
								longListCluster.append(lista_cluster[n])
							n = n + 1
					elif r==b and r == g and r >200 and r <210 : # se entro qui dovrei essere in scala di grigio, e pixel grigio
						greyPixelCount+=1
			#--------aggiungo parte dove faccio media del cluster dominante #nel caso questa cosa va aggiunta anche dalle altre parti
			#TODO: questa funzione dovrebbe essere modificata come trova_spazi_gt_da_frontiere_versione_2 di metriche frontiere
			pmax = 0
			for elem in lista_cluster:
				if longListCluster.count(elem) > pmax:
					pmax = longListCluster.count(elem)
					cluster = elem			
			#--------------------			
			#print "il numero di n_elementi_cluster", n_elementi_cluster
			#if pixel_diversi >= 0.3 * n_elementi_cluster and n_elementi_cluster > 0: #la copertura deve essere il 30% di quel cluster
			if pmax >= soglia_pixel: #elimino un po' di rumore
				celletta.set_vedo_frontiera(True)
				celletta.set_celletta_out(False)
				celletta.set_celletta_parziale(True)	
				celletta.set_cluster_frontiera(cluster)
				celletta.set_pixel_cluster_frontiera(pmax)
				celletta.set_grey_pixel_count(greyPixelCount)
			else:
				celletta.set_vedo_frontiera(False)
				celletta.set_celletta_out(False)
				celletta.set_celletta_parziale(False)
...
def assegna_cluster_frontiera_a_lista_celle(lista_celle, lista_r,lista_g,lista_b,lista_cluster,immagine_cluster):
	img_flipt= cv2.flip(immagine_cluster,0)
	soglia_pixel = 5
	for celletta in lista_celle:
		if celletta.already_checked == False:
			bordo = celletta.cella.bounds #restituisce una tupla del tipo (minx, miny, maxx, maxy)
			#print bordo
			bordo = [int(i) for i in bordo]
			#print "nuovo",bordo
			...
			if pmax >= soglia_pixel: #elimino un po' di rumore
				...
				celletta.set_vedo_frontiera(False)
				celletta.set_celletta_parziale(False)
				celletta.set_cluster_frontiera(None)
			celletta.set_already_checked(True)
	return lista_celle
		...
		for i in xrange(0, altezza):
			for j in xrange( 0, larghezza):
				...
				if r!=g or r!=b or g!=b:#se ci sono componenti rgb diverse allora non sono in scala di grigio e quel pixel fa parte della frontiera
					...
					greyPixelCount+=1
		#print "il numero di n_elementi_cluster", n_elementi_cluster
		if pixel_diversi >soglia_pixel : #se nell'immagine ci sono almeno 20 pixel della frontiera altrimenti e' solo rumore
			celletta.set_vedo_frontiera(True)
			#celletta.set_celletta_out(False)
			celletta.set_celletta_parziale(True)
			celletta.set_grey_pixel_count(greyPixelCount)	
...
def assegna_area_predetta_celle_parziali(plan_o, lista_r,lista_g,lista_b,lista_cluster, immagine_cluster):
	'''
	terzo metodo per riconoscere le stanze parziali, basato sul riconoscimento delle frontiere.
	Questo metodo controlla che all'interno dei pixel di una cella ci siano almeno il 30% di pixel dello stesso cluster per fare in modo che quella cella sia una cella parziale
	'''
	...
	spazi = plan_o.spazi #oggetto di tipo spazio
	img_flipt= cv2.flip(immagine_cluster,0) #capire se va usato sempre
	number_of_cluster = len(lista_cluster)
	for spazio in spazi:
		for celletta in spazio.cells:
				...
				for j in xrange( 0, larghezza):
					...
					if r!=g or r!=b or g!=b:#se ci sono componenti rgb diverse allora non sono in scala di grigio e quel pixel fa parte della frontiera
						...
						while n < len(lista_r) and trovato == False:
							...
							if r == re and g == gr and b == bl:
								trovato = True
								cluster =lista_cluster[n]
								trovato_2 = False
								n_2 = 0
								
								while trovato_2 == False and n_2<len(pointListObject_list):
									if(pointListObject_list[n_2].cluster == cluster):
										trovato_2 = True
										pointListObject_list[n_2].add_point((j,i))
									n_2= n_2 +1
								if trovato_2 == False:
									lo = ListOfPoint(cluster)
									lo.add_point((j,i))
									pointListObject_list.append(lo)
							n=n+1
						'''
			celletta.set_grey_pixel_count(greyPixelCount)

			
def calcola_grey_pixel_celletta(celletta,immagine_cluster):
	img_flipt= cv2.flip(immagine_cluster,0)
	if celletta.already_checked == False:
		bordo = celletta.cella.bounds #restituisce una tupla del tipo (minx, miny, maxx, maxy)
		#print bordo
		...
		for i in xrange(0, altezza):
			for j in xrange( 0, larghezza):
				...
				g = immagine_cella[i][j][1]
				b = immagine_cella[i][j][2]
				if r==b and r == g and r >200 and r <210 : # se entro qui dovrei essere in scala di grigio, e pixel grigio
					greyPixelCount+=1
	else:
		greyPixelCount = celletta.grey_pixel_count

	return greyPixelCount
