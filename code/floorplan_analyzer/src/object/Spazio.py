...
class Celletta(object):
	def __init__(self, polygon_cella, cella_normale):
		...
		self.already_checked = False #Usato quando devi assegnare un cluster frontiera ad una cella, controllata se e' gia' stata controllata 
		self.grey_pixel_count = 0
	...
	def set_already_checked(self,bool):
		self.already_checked = bool
	def set_pixel_cluster_frontiera(self,pixel_cluster):
		self.pixel_cluster_frontiera = pixel_cluster #usato per memorizzare quanti pixel della cella appartengono alla frontiera. Usato per decidere successivamente a quale cluster va assegnata la stanza
	def set_grey_pixel_count(self,number):
		self.grey_pixel_count = number
...
def trova_spazi_parziali_conta_pixel(spazi):
	'''
	Metodo per controllare se uno spazio e' parziale oppure no controllando le celle.
	Viene assegnato il cluster che 'conta' piu' pixel
	'''
	for s in spazi:
		c = []
		pixel_cluster = []
		...
		for cella in s.cells:
			if cella.parziale == True:
				trovato = True
				if c.count(cella.cluster_frontiera) == 0:
					c.append(cella.cluster_frontiera)
					pixel_cluster.append(cella.pixel_cluster_frontiera)
				else:
					for i in range(len(c)):
						if c[i] == cella.cluster_frontiera:
							pixel_cluster[i] = pixel_cluster[i] + cella.pixel_cluster_frontiera
		s.set_parziale(trovato)
		max = 0
		for i in range(len(c)):
			co = pixel_cluster[i]
			if co > max:
		...
		s.set_cluster_frontiera(cluster)
